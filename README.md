# NeuraPress — Autonomous Multi-Agent AI Blog Generator
...
uv run python -m neurapress.run --dry-run

# Serve the frontend (uses bundled sample articles):
uv run python -m http.server 8000 --directory neurapress/web
# → http://localhost:8000
```

The default storage backend is **local** (`neurapress/web/blogs/`) — no cloud account needed.
...
| Component | Role |
|---|---|
| `neurapress/graph/` | LangGraph state machine + routing/revision logic |
| `neurapress/agents/` | TutorialAgent, NewsAgent, ValidatorAgent (+ prompts) |
| `neurapress/services/llm.py` | Groq ChatGroq wrapper + ReAct agent factory |
| `neurapress/services/storage.py` | Storage factory: **R2** (S3 API) or **local** filesystem |
| `neurapress/services/site_services.py` | RSS 2.0 + sitemap.xml generation |
| `neurapress/tools/` | Tavily, Guardian, Unsplash API tools |
| `neurapress/api/app.py` | FastAPI admin API (health/stats/articles/trigger) |
| `neurapress/web/` | Static frontend (vanilla JS, no build step) |
...
uv run python -m neurapress.run

# Force a domain:
uv run python -m neurapress.run --domain nlp

# Weekly news roundup:
uv run python -m neurapress.run --ainews

# Preview without keys/writes:
uv run python -m neurapress.run --dry-run

# Admin API:
uv run uvicorn neurapress.api.app:app --port 8001
