...
from neurapress.agents.validator_agent.agent import _extract_json, validator_node
...
class TestValidatorSaveFlow:
    def _run_approved(self, tmp_path: Path, state: dict, llm_payload: str):
        ...
        def fake_invoke(prompt, attempts=4):
            return FakeRes()

        fake_service = type("FakeService", (), {"invoke": staticmethod(fake_invoke)})()
        with (
            patch("neurapress.agents.validator_agent.agent.LLMAgentService", return_value=fake_service),
            patch("neurapress.agents.validator_agent.agent.get_storage", return_value=storage),
            patch("neurapress.agents.validator_agent.agent.fetch_cover_image", return_value=None),
        ):
    ...
    def test_upload_failure_does_not_register(self, tmp_path: Path):
        ...
        with (
            patch("neurapress.agents.validator_agent.agent.LLMAgentService", return_value=fake_service),
            patch(
                "neurapress.agents.validator_agent.agent.get_storage",
